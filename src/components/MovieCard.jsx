    ...
    return (
        <Link to={`/movie/${movie.id}`} className="group block w-full relative">
            <div className="relative aspect-[3/4] rounded-3xl overflow-hidden mb-3">
                ...
                {/* Rating Badge */}
                <div className="absolute top-3 right-3 bg-black/40 backdrop-blur-md border border-white/10 px-2 py-1 rounded-lg text-[10px] font-bold text-yellow-400 flex items-center gap-1">
                    <span>⭐</span> {movie.rating}
                </div>
            </div>

            <h3 className="font-bold text-white text-base truncate pr-2 mb-1">{movie.title}</h3>
